...
public class MandatoryWarningHandler {
    ...
    public void report(Lint lint, DiagnosticPosition pos, LintWarning warnKey) {
        ...
        // We assume the use of a consistent LintCategory
        Assert.check(lintCategory == null || lintCategory == warnKey.getLintCategory());
        lintCategory = warnKey.getLintCategory();

        // If the category is explicitly suppressed, don't log anything, but do validate the suppression
        if (lint.isSuppressed(lintCategory, true))
            return;
    ...
    public void reportDeferredDiagnostic() {
        if (deferredDiagnosticKind != null) {
            String prefix = lintCategory.option;
     ...
     * This is inferred from the key of the first warning that is logged.
     */
    private LintCategory lintCategory;
